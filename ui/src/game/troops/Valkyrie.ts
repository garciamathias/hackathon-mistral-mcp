...
export interface ValkyriePosition {
  row: number;
  col: number;
}
...
export interface Valkyrie {
  id: string;
  type: TroopType.VALKYRIE;
  team: 'red' | 'blue';
  position: ValkyriePosition;
  targetPosition: ValkyriePosition;
  state: ValkyrieState;
  health: number;
  maxHealth: number;
  speed: number; // cellules par seconde
  isAlive: boolean;
  bridgeTarget?: ValkyriePosition;
...
// Positions des ponts
export const BRIDGES: ValkyriePosition[] = [
  { row: 16, col: 3 },
  { row: 16, col: 14 }
];

export class ValkyrieEntity {
  public data: Valkyrie;

  constructor(id: string, team: 'red' | 'blue', customPosition?: ValkyriePosition) {
    const defaultPosition = { row: 0, col: 0 };
    const startPosition = customPosition || defaultPosition;
    const config = TROOP_CONFIGS[TroopType.VALKYRIE];
    ...
    console.log(`Valkyrie ${team} spawned at (${startPosition.row}, ${startPosition.col}) - focusOnBuildings: ${config.focusOnBuildings}, flying: ${config.flying}`);
  }
  ...
  public update(deltaTime: number, activeTowers: Tower[], flaggedCells?: Set<string>, gameEngine?: GameEngine): void {
    ...
    switch (this.data.state) {
      case ValkyrieState.SPAWNING:
        ...
        this.data.state = newState as unknown as ValkyrieState;
      ...
      case ValkyrieState.MOVING_TO_BRIDGE:
        ...
        this.data.state = bridgeState as unknown as ValkyrieState;
        break;

      case ValkyrieState.SEEKING_TARGET:
        ...
        if (!this.data.isInCombat && gameEngine) {
          ...
          if (closestEnemyResult && this.data.towerTarget !== closestEnemyResult.target.id) {
            ...
            if (closestEnemyResult.distance < currentDistance - 1.0) {
              console.log(`Valkyrie ${this.data.id} retargeting from ${this.data.towerTarget} to closer enemy ${closestEnemyResult.target.id}`);
        ...
        if (this.data.towerTarget) {
          ...
          if (distanceToTarget <= 1.0) { // Range d'attaque courte pour Valkyrie (attaque de mêlée)
            console.log(`Valkyrie ${this.data.id} entering combat mode! Distance: ${distanceToTarget.toFixed(2)}`);
            this.data.isInCombat = true;
            this.data.state = ValkyrieState.ATTACKING_TARGET;
          } else {
            // Vérifier si la Valkyrie est bloquée par les flagged cells de sa cible
            ...
            if (flaggedCells && targetTower && this.isBlockedByTargetFlaggedCells(flaggedCells, targetTower)) {
              console.log(`Valkyrie ${this.data.id} is blocked by target's flagged cells, entering combat mode!`);
              this.data.isInCombat = true;
              this.data.state = ValkyrieState.ATTACKING_TARGET;
            } else {
              // Se déplacer vers la cible en vérifiant si on arrive à portée pendant le mouvement
              this.moveTowardsWithRangeCheck(this.data.targetPosition, deltaTime, flaggedCells, 1.0);
            }
          }
        }
        break;

      case ValkyrieState.ATTACKING_TARGET:
        ...
        // Vérifier si la Valkyrie est bloquée par les flagged cells
        const targetTower = activeTowers.find(tower => tower.id === this.data.towerTarget);
  ...
  private isTargetStillValid(activeTowers: Tower[], gameEngine?: GameEngine): boolean {
    ...
    if (targetTower) {
      if (!targetTower.isAlive) {
        console.log(`Valkyrie ${this.data.id} target tower ${this.data.towerTarget} is dead`);
        return false;
      }
      return true;
    }
  ...
  private moveTowardsWithRangeCheck(target: ValkyriePosition, deltaTime: number, flaggedCells?: Set<string>, stopRange?: number): void {
    ...
    if (stopRange && distance <= stopRange) {
      console.log(`Valkyrie ${this.data.id} stopping movement - in range! Distance: ${distance.toFixed(2)}`);
      this.data.isInCombat = true;
      this.data.state = ValkyrieState.ATTACKING_TARGET;
      return;
    }
    ...
    // Valkyrie n'est pas volante, donc vérifie les flagged cells
    if (!this.data.flying && flaggedCells && this.wouldCollideWithFlaggedCell(newPosition, flaggedCells)) {
      console.log(`Valkyrie ${this.data.id} (${this.data.team}) blocked by flagged cell`);
  ...
  private moveTowards(target: ValkyriePosition, deltaTime: number, flaggedCells?: Set<string>): void {
    const { position, speed } = this.data;
  ...
  private wouldCollideWithFlaggedCell(position: ValkyriePosition, flaggedCells: Set<string>): boolean {
    const cellKey = `${Math.floor(position.row)}-${Math.floor(position.col)}`;
    return flaggedCells.has(cellKey);
  }

  private calculateSmartAvoidance(position: ValkyriePosition, target: ValkyriePosition, flaggedCells: Set<string>, moveDistance: number): {dx: number, dy: number} | null {
  ...
  private findObstacleCenter(position: ValkyriePosition, flaggedCells: Set<string>): ValkyriePosition | null {
    const nearbyFlaggedCells: ValkyriePosition[] = [];
  ...
  private isAtPosition(target: ValkyriePosition): boolean {
  ...
  private updateTargetPosition(activeTowers: Tower[], gameEngine?: GameEngine): void {
    ...
    if (target) {
      ...
      console.log(`Valkyrie ${this.data.id} target ${this.data.towerTarget} disappeared, clearing target`);
      this.data.towerTarget = undefined;
    }
  }
  ...
  private attackTarget(deltaTime: number, activeTowers: Tower[], gameEngine?: GameEngine, isBlockedByFlaggedCells?: boolean): void {
    ...
    if (!isBlockedByFlaggedCells && distanceToTarget > attackRange) {
      ...
      console.log(`Valkyrie ${this.data.id} target too far (${distanceToTarget.toFixed(2)}), switching back to seeking mode`);
      this.data.state = ValkyrieState.SEEKING_TARGET;
      this.data.isInCombat = false;
      return;
    }
    ...
    if (timeSinceLastAttack >= attackCooldown) {
      console.log(`Valkyrie ${this.data.id} attacks ${target.type} ${this.data.towerTarget} for ${this.data.attackDamage} damage!`);
  ...
  public takeDamage(damage: number): void {
    ...
    console.log(`Valkyrie ${this.data.id} takes ${damage} damage! Health: ${this.data.health}/${this.data.maxHealth}`);
    
    if (this.data.health <= 0) {
      this.data.health = 0;
      this.data.isAlive = false;
      this.data.state = ValkyrieState.DEAD;
      console.log(`Valkyrie ${this.data.id} has been defeated!`);
    }
  }
  ...
  private isBlockedByTargetFlaggedCells(flaggedCells: Set<string>, targetTower: any): boolean {
    ...
    // Vérifier si la position actuelle de la Valkyrie est une flagged cell de la tour cible
    const currentCellKey = `${Math.floor(this.data.position.row)}-${Math.floor(this.data.position.col)}`;
    
    // Si la Valkyrie est sur une flagged cell, c'est qu'elle est bloquée par sa cible
    ...
    // Si la Valkyrie est à côté d'une flagged cell, elle est probablement bloquée
